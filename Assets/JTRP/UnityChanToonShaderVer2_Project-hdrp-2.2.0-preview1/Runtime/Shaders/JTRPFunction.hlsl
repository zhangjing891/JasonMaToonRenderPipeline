	...
	void GetSSRimLight(inout float3 color, PositionInputs posInput, float2 uv, float3 lightDirWS, float3 normalDirWS, half shadowValue)
	{
		...
		float2 L_View = normalize(mul((float3x3)UNITY_MATRIX_V, lightDirWS).xy) * (_SSRimInvertLightDir ? - 1 : 1);	//LdirVS
		float2 N_View = normalize(mul((float3x3)UNITY_MATRIX_V, normalDirWS).xy);	//NdirVS
		float lDotN = saturate(dot(N_View, L_View) + _SSRimLength);
		
		float scale = mask.r * widthRamp * lDotN * _SSRimWidth * GetScaleWithHight();		//GetScaleWithHight()将屏幕坐标的Y轴映射到[0,1]
		
		float2 ssUV1 = clamp(posInput.positionSS + N_View * scale, 0, _ScreenParams.xy - 1);	//偏移
		float viewDepth = distance(posInput.positionWS, GetCameraRelativePositionWS(_WorldSpaceCameraPos));		//世界空间顶点位置与相机到世界空间顶点位置的距离
		
		float3 sceneWorldPos = GetWorldPosFromDepthBuffer(ssUV1 * _ScreenSize.zw, LoadCameraDepth(ssUV1));
		float sceneViewDepth = distance(GetCameraRelativePositionWS(sceneWorldPos), GetCameraRelativePositionWS(_WorldSpaceCameraPos));	//世界空间顶点位置与从深度图中转的世界空间顶点位置的距离

		float intensity = smoothstep(viewDepth, viewDepth + _SSRimFeather, sceneViewDepth);		//sceneViewDepth<= viewDepth为0
		// color = GetCameraRelativePositionWS(_WorldSpaceCameraPos);
		intensity *= mask.a * lerp(1, _SSRimInShadow, shadowValue) * _SSRimIntensity;			//shadowValue = 阴影区域
